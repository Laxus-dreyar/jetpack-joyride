        ...
        for i in range(self.__rows):
            for j in range(start,start+columns):
                if (i == x+1 or i == x or i==x-1) and (j == y+1+self.__curscreen or j == y+self.__curscreen or j == y+self.__curscreen-1):
                    print(Fore.WHITE + self.__grid[i][j],end='')
                elif self.__grid[i][j] == '$':
                    print(Fore.YELLOW + '$',end='')
                elif self.__grid[i][j] == '-' or self.__grid[i][j] == '/' or self.__grid[i][j] == '|' or self.__grid[i][j] == '*':
                    print(Fore.RED + str(self.__grid[i][j]),end='')
                elif self.__grid[i][j] == 'M' or self.__grid[i][j] == 'F':
                    print(Fore.BLUE + str(self.__grid[i][j]),end='')
                else:
                    print(Fore.WHITE + str(self.__grid[i][j]),end='')
            print()
